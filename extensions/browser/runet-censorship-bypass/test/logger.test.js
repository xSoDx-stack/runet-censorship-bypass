...
describe('Logger: sanitizeLogString & sanitizeLogData', () => {
  ...
  it('does not aggregate entries with different explicit group keys', () => {
    const originalScheduleSave = logger.scheduleSave;
    logger.resetRuntimeState();
    logger.scheduleSave = () => {};

    try {
      const first = logger.add({
        level: 'warn',
        category: 'proxy',
        title: 'Tunnel failure',
        message: 'Request failed',
        groupKey: 'docs.github.com|main_frame',
      });
      const repeated = logger.add({
        level: 'warn',
        category: 'proxy',
        title: 'Tunnel failure',
        message: 'Request failed',
      ...
      logger.add({
        ...
        title: 'Tunnel failure',
        message: 'Request failed',
        groupKey: 'github.com|main_frame',
      });

      expect(repeated).to.equal(first);
      expect(first.count).to.equal(2);
      expect(logger.getLogs()).to.have.lengthOf(2);
    } finally {
      logger.scheduleSave = originalScheduleSave;
      logger.resetRuntimeState();
    }
